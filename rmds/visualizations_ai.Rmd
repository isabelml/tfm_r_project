...
```{r fig.height=4.5, fig.width=6.7}
...
tokens %>%
  ...
  labs(title = "Word frequency in artificial intelligence articles published in eldiario.es",
       subtitle = "2014-2023",
...
tokens %>%
  ...
  labs(title = "Word frequency in artificial intelligence articles published in elmundo.es",
       subtitle = "2014-2023",
...
Now I am going to quantify how similar are both datasets (elDiario.es and El Mundo) with
...
In this section I will conduct a sentiment analysis of the speeches. 
...
# To see the sentiments that include NRC lexicon: 
unique(sentiment_spanish_nrc$sentiment) 
```
### Visualization of the eight sentiments in elDiario.es articles
...
### Trust words in the articles: NRC lexicon

I want to analyze the use of words of anticipation used in the articles.

First I filter in the dataframe created before with the NRC lexicon the words 
classified with the sentiment "anticipation".

```{r}
trust_words <- sentiment_spanish_nrc %>% 
  filter(sentiment == "fear")

trust_words
```

Then I put it together with the dataframe of the tokenized speeches and count the
frequency of each word. In the following dataframe they are sorted from most to 
least frequent.

```{r}
np_trust_words <- tokens %>%
    inner_join(trust_words, by = "word") %>%
    count(word, sort = TRUE)

np_trust_words
```

### Visualization of fear words comparing elDiario.es and El Mundo
...
In order to use the AFINN lexicon in Spanish, I have first downloaded it (see code). 
This chunk is not executable since the csv will be read later.
...
Let's prepare the lexicon to use it:

```{r}
 afinn <- read.csv("../data/lexico_afinn.en.es.csv", stringsAsFactors = F, fileEncoding = "latin1") %>% 
  tbl_df() %>% 
  rename(word = Palabra,
         value = Puntuacion) %>% 
  select(-Word)
...
head(afinn)
```
I am going to create a estimate of the net sentiment (sum of values) in 
each month for AFINN sentiment lexicon. Let's look at them:

```{r}
afinn_eldiario <- tokens %>% 
  filter(newspaper == "elDiario.es") %>% 
...
```{r}
afinn_eldiario %>% 
    # mutate(month = factor(as.factor(month), levels = c("January",
    #                                                  "February", "March",
    #                                                  "April", "May", "June",
...
Finally, I want to see the most repeated words in the articles
in a word cloud.
