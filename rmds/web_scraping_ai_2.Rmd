...
# 13. Merge both dataframes

```{r}
write_csv(eldiario_df, "../data/eldiario_text2.csv")
write_csv(elmundo_df, "../data/elmundo_text2.csv")

eldiario_df_all <- eldiario_df %>% 
  full_join(df2_eldiario, by = "link")

elmundo_df_all <- elmundo_df %>% 
  full_join(df2_elmundo, by = "link") %>% 
  filter(!duplicated(title))
  
```

# 14. Save the dataframes in a csv

```{r}
write_csv(eldiario_df, "eldiario_news.csv")
write_csv(elmundo_df_all, "../data/elmundo_news_all.csv")

a <- read_csv("../data/eldiario_text2.csv")
```
